...
export function applyLocalCommand(atom: AtomView, operation: "confirm" | "edit" | "rescope" | "limit" | "revoke", editedClaim?: string): AtomView {
  ...
  const claim = operation === "edit" && editedClaim?.trim() ? editedClaim.trim() : atom.claim;
  return { ...atom, version: atom.version + 1, lifecycle, scope, claim, mode: operation === "rescope" ? "engineering" : atom.mode };
}
...
export function App() {
  ...
  const command = async (operation: "confirm" | "edit" | "rescope" | "limit" | "revoke") => {
    if (!atom) return;
    const editedClaim = operation === "edit" ? (draftClaim || globalThis.prompt("Edit learning claim", atom.claim) || atom.claim) : undefined;
    const next = applyLocalCommand(atom, operation, editedClaim);
  ...
  return <main className="studio-shell">
    ...
    <div className="workspace"><aside><h2>Learning queue</h2>{atoms.map((item) => <button className={item.id === atom?.id ? "atom selected" : "atom"} key={item.id} onClick={() => setSelected(item.id)}><b>{item.claim}</b><small>{item.facet} · {item.lifecycle} · {Math.round(item.confidence * 100)}% confidence</small></button>)}</aside>
      {atom && <article><div className="card-head"><div><span className={`pill ${atom.lifecycle}`}>{atom.lifecycle}</span><span className="version">version {atom.version}</span></div><span className="mode">mode: {atom.mode ?? "all"}</span></div><h2>{atom.claim}</h2><p className="muted">Narrow lesson extracted from explicit user evidence. It will only be eligible when task scope and mode match.</p>
        ...
        <label className="edit-field">Claim editor<input value={draftClaim || atom.claim} onChange={(event) => setDraftClaim(event.target.value)} aria-label="Learning claim" /></label><div className="actions"><button onClick={() => void command("confirm")}>Approve</button><button onClick={() => void command("edit")}>Save claim</button><button onClick={() => void command("rescope")}>Rescope</button><button onClick={() => void command("limit")}>Limit to task</button><button className="danger" onClick={() => void command("revoke")}>Revoke</button></div><p className="notice" role="status">{notice}</p>
      </article>}
    </div>
    <style>{styles}</style>
  </main>;
