...
const atom = (id: string, patch: Partial<ProfileAtom> = {}): ProfileAtom => ({
  ...
  subject_id: "user-a",
  facet: "communication",
  claim: "For email, use concise subjects.",
...
describe("guidance compiler", () => {
  it("is byte deterministic and gives provenance for included and excluded atoms", () => {
    const atoms = [
      atom("included"),
      atom("wrong-project", { scope: { level: "project", ref: "other" } }),
      atom("wrong-mode", { mode: "creative" }),
      atom("revoked", { lifecycle: "revoked" })
    ];
    ...
    expect(result.candidates.find((candidate) => candidate.atom.id === "wrong-project")?.reasons).toContain(
      "scope_mismatch"
    );
    expect(result.candidates.find((candidate) => candidate.atom.id === "wrong-mode")?.reasons).toContain(
      "mode_mismatch"
    );
    expect(result.candidates.find((candidate) => candidate.atom.id === "revoked")?.reasons).toContain(
      "inactive_or_revoked"
    );
  });
