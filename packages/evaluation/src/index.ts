...
export interface CausalEvaluationReport {
  ...
  readonly arms: Readonly<
    Record<CausalArm, { readonly related: RunOutput; readonly unrelated: RunOutput; readonly tokenCount: number }>
  >;
...
export class LearningLedger {
  ...
  relevant(task: string, mode: string): Lesson | null {
    return (
      [...this.lessons.values()].find((x) => x.lifecycle === "approved" && x.task === task && x.mode === mode) ?? null
    );
  }

  snapshot() {
    return { evidence: [...this.evidence.values()], lessons: [...this.lessons.values()] };
  }
...
export class JsonLearningLedger {
  ...
  async save(ledger: LearningLedger) {
    const tmp = `${this.path}.tmp`;
    await writeFile(tmp, JSON.stringify(ledger.snapshot(), null, 2), "utf8");
    await rename(tmp, this.path);
  }
  async load() {
    try {
      return LearningLedger.fromSnapshot(
        JSON.parse(await readFile(this.path, "utf8")) as ReturnType<LearningLedger["snapshot"]>
      );
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return new LearningLedger();
      throw error;
    }
  }
}
...
function run(input: RunInput, variant: Variant, lesson: Lesson | null): RunOutput {
  const applicable =
    variant === "full" &&
    lesson &&
    lesson.lifecycle === "approved" &&
    lesson.task === input.task &&
    lesson.mode === input.mode
      ? lesson
      : null;
  ...
  return {
    variant,
    guidanceApplied: applicable?.guidance ?? null,
    answer,
    correct,
    trace: applicable ? ["lesson", applicable.evidenceId] : ["baseline"]
  };
}

export function evaluateControlled(scenario = "formatting-correction"): EvaluationReport {
  const ledger = new LearningLedger();
  const evidence = ledger.appendCorrection({
    id: "e-correction-1",
    task: "formatting",
    mode: "editorial",
    statement: "Use concise bullets for this task.",
    createdAt: "2026-01-01T00:00:00.000Z"
  });
  const proposed = ledger.proposeLesson({
    id: "l-bullets-1",
    evidenceId: evidence.id,
    task: "formatting",
    mode: "editorial",
    guidance: "Use bullets",
    scope: "task",
    confidence: 0.98
  });
  ...
  const ablation = Object.fromEntries(
    (["baseline", "full", "no-evidence", "no-retrieval", "no-learning"] as Variant[]).map((v) => [
      v,
      run(related, v, v === "no-evidence" || v === "no-retrieval" || v === "no-learning" ? null : lesson)
    ])
  ) as Record<Variant, RunOutput>;
  const rb = run(related, "baseline", null),
    rc = run(related, "full", lesson),
    ub = run(unrelated, "baseline", null),
    uc = run(unrelated, "full", lesson);
  return {
    scenario,
    related: { baseline: rb, conditioned: rc },
    unrelated: { baseline: ub, conditioned: uc },
    ablation,
    improvement: !rb.correct && rc.correct,
    unrelatedUnaffected: ub.answer === uc.answer,
    attribution:
      !rb.correct && rc.correct && rc.trace.includes(evidence.id) ? "retrieved-approved-lesson" : "not-attributed"
  };
}
...
export function evaluateCausalComparison(scenario = "formatting-correction"): CausalEvaluationReport {
  const ledger = new LearningLedger();
  const evidence = ledger.appendCorrection({
    id: "causal-evidence-1",
    task: "formatting",
    mode: "editorial",
    statement: "Use bullets",
    createdAt: "2026-01-01T00:00:00.000Z"
  });
  const proposed = ledger.proposeLesson({
    id: "causal-lesson-1",
  ...
  const rawMemory = (input: RunInput): RunOutput => ({
    variant: "baseline",
    guidanceApplied: lesson.guidance,
    answer: `${lesson.guidance} :: ${input.prompt}`,
    correct: input.task === "formatting",
    trace: ["raw_memory", evidence.id]
  });
  ...
  const merakiPreferred =
    arms.meraki_pack.related.correct &&
    !arms.meraki_pack.unrelated.guidanceApplied &&
    Boolean(arms.raw_memory.unrelated.guidanceApplied);
  return {
    scenario,
    arms,
...
export async function verifyRestart(path: string): Promise<boolean> {
  const store = new JsonLearningLedger(path),
    ledger = new LearningLedger();
  const evidence = ledger.appendCorrection({
    id: "restart-evidence",
    task: "formatting",
    mode: "editorial",
  ...
  const lesson = ledger.proposeLesson({
    id: "restart-lesson",
    evidenceId: evidence.id,
    task: "formatting",
    mode: "editorial",
  ...
  ledger.governLesson(lesson.id, "approved");
  await store.save(ledger);
  const restored = await store.load();
  return restored.relevant("formatting", "editorial")?.evidenceId === evidence.id;
}
