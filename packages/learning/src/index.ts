...
import type { Event, EvidenceRef, Feedback, MerakiPack, ProfileAtom, RetrievalCandidate, Scope, SourceRecord, TaskContext, UpdateProposal } from "@meraki/contracts";
import { EvidenceLedger, type EvidenceChain, type EvidenceLedgerSnapshot, type ExplicitActivityInput, type ModelOutputInput, type ObjectiveOutcomeInput } from "@meraki/evidence";
import { compileGuidance } from "@meraki/guidance";
import { ProfileGraph } from "@meraki/profile";
...
export type Lesson = ProfileAtom & Readonly<{ guidance: string; sourceEventId: string; observationId: string; signalId: string; hypothesisId: string }>;
export type ActivityLessonInput = Readonly<{ eventId: string; claim: string; facet?: ProfileAtom["facet"]; temporalHorizon?: ProfileAtom["temporal_horizon"] }>;
...
export type GovernedUpdateProposal = Readonly<{ proposal: UpdateProposal; targetBefore: ProfileAtom; evidenceEventId: string; appliedVersion?: number }>;
export type UpdateOperation = Extract<UpdateProposal["operation"], "reinforce" | "weaken">;
export type LearningEngineSnapshot = Readonly<{ evidence: ImmutableCorrection[]; lessons: Lesson[]; evidenceLedger: EvidenceLedgerSnapshot; profile: ReturnType<ProfileGraph["snapshot"]>; updateProposals?: GovernedUpdateProposal[] }>;
...
export class LearningEngine {
  ...
  recordCorrection(input: CorrectionInput): ImmutableCorrection {
    ...
    const evidence: ImmutableCorrection = Object.freeze({ feedback, source: chain.source, event: chain.event, eventId, original: input.original, correction: input.correction, taskType: input.taskType, scope: input.scope, ...(input.mode === undefined ? {} : { mode: input.mode }), contentHash });
  ...
  recordModelOutput(input: ModelOutputInput): EvidenceChain { return this.evidenceLedger.ingestModelOutput(input); }
  ...
  extractLesson(eventId: string): Lesson {
    ...
    const hypothesis = this.evidenceLedger.proposeHypothesis(signal.id, `For ${evidence.taskType}, prefer: ${evidence.correction}`);
    const atom = this.profile.createCandidate({ tenantId: evidence.feedback.tenant_id, subjectId: evidence.feedback.subject_id, facet: "workflow", claim: hypothesis.claim, epistemicClass: "declared", scope: evidence.scope, ...(evidence.mode === undefined ? {} : { mode: evidence.mode }), temporalHorizon: "ongoing", evidence: [ref(evidence.eventId, evidence.correction)] });
    const lesson: Lesson = Object.freeze({ ...atom, guidance: hypothesis.claim, sourceEventId: evidence.eventId, observationId: observation.id, signalId: signal.id, hypothesisId: hypothesis.id });
  ...
  extractActivityLesson(input: ActivityLessonInput): Lesson {
    ...
    if (source.trust_class !== "explicit_user" && source.trust_class !== "objective_outcome") throw new Error("ACTIVITY_LESSON_TRUST_REQUIRED");
    const scope = event.payload.scope;
    if (!scope || typeof scope !== "object" || typeof (scope as Scope).level !== "string" || typeof (scope as Scope).ref !== "string") throw new Error("ACTIVITY_SCOPE_REQUIRED");
    ...
    const atom = this.profile.createCandidate({ tenantId: event.tenant_id, subjectId: event.subject_id, facet: input.facet ?? "workflow", claim: hypothesis.claim, epistemicClass: source.trust_class === "objective_outcome" ? "objective" : "observed", scope: scope as Scope, ...(typeof event.payload.mode === "string" ? { mode: event.payload.mode } : {}), temporalHorizon: input.temporalHorizon ?? "ongoing", evidence: [...hypothesis.evidence] });
    const lesson: Lesson = Object.freeze({ ...atom, guidance: atom.claim, sourceEventId: event.id, observationId: observation.id, signalId: signal.id, hypothesisId: hypothesis.id });
    this.lessons.set(atom.id, lesson);
    return lesson;
  }
  ...
  proposeUpdate(lessonId: string, evidenceEventId: string, operation: UpdateOperation): UpdateProposal {
    ...
    const proposal: UpdateProposal = Object.freeze({ contract: "update_proposal", id: randomUUID(), tenant_id: lesson.tenant_id, subject_id: lesson.subject_id, operation, target: { id: lessonId, version: targetBefore.version }, evidence: [evidence], expected_impact: operation === "reinforce" ? "Increase utility only for this already-scoped active atom." : "Reduce confidence only for this atom using the cited counterevidence.", status: "pending", expected_version: targetBefore.version, created_at: now() });
  ...
  applyUpdateProposal(proposalId: string): { proposal: UpdateProposal; atom: ProfileAtom } {
    ...
    const atom = record.proposal.operation === "reinforce"
  ...
  rollbackUpdateProposal(proposalId: string): { proposal: UpdateProposal; atom: ProfileAtom } {
    const record = this.requireUpdateProposal(proposalId);
    if (record.proposal.status !== "applied" || record.appliedVersion === undefined) throw new Error("UPDATE_PROPOSAL_NOT_APPLIED");
  ...
  learningTrace(eventId: string): LearningTrace {
    ...
    if (!lesson) return { source, event, ...(discoveredObservation === undefined ? {} : { observation: { id: discoveredObservation.id, description: discoveredObservation.description, epistemicClass: discoveredObservation.epistemic_class } }) };
    ...
    return { source, event, observation: { id: observation.id, description: observation.description, epistemicClass: observation.epistemic_class }, signal: { id: signal.id, kind: signal.kind, support: signal.support, confidence: signal.confidence }, hypothesis: { id: hypothesis.id, claim: hypothesis.claim, confidence: hypothesis.confidence }, atom: this.profile.current(lesson.id) };
  }
  ...
  getProfileAtoms(): ProfileAtom[] { return this.profile.all(); }
  updateProposals(): UpdateProposal[] { return [...this.updateProposalRecords.values()].map((record) => record.proposal).sort((left, right) => right.created_at.localeCompare(left.created_at)); }
  getProfileRevisions(lessonId: string): readonly ProfileAtom[] { return this.profile.revisions(lessonId); }
  snapshot(): LearningEngineSnapshot { return { evidence: [...this.evidence.values()], lessons: [...this.lessons.values()], evidenceLedger: this.evidenceLedger.snapshot(), profile: this.profile.snapshot(), updateProposals: [...this.updateProposalRecords.values()] }; }
  static fromSnapshot(snapshot: LearningEngineSnapshot): LearningEngine {
    ...
    for (const proposal of snapshot.updateProposals ?? []) engine.updateProposalRecords.set(proposal.proposal.id, Object.freeze(proposal));
...
export class JsonLearningEngineStore {
  constructor(private readonly path: string) {}
  async save(engine: LearningEngine): Promise<void> { const temporary = `${this.path}.tmp`; await writeFile(temporary, JSON.stringify(engine.snapshot(), null, 2), "utf8"); await rename(temporary, this.path); }
  async load(): Promise<LearningEngine> { try { return LearningEngine.fromSnapshot(JSON.parse(await readFile(this.path, "utf8")) as LearningEngineSnapshot); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return new LearningEngine(); throw error; } }
}
