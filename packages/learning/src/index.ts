...
export class JsonLearningEngineStore {
  ...
  async save(engine: LearningEngine): Promise<void> {
    const temporary = `${this.path}.tmp`;
    await writeFile(temporary, JSON.stringify(engine.snapshot(), null, 2), "utf8");
    await rename(temporary, this.path);
  }
  async load(): Promise<LearningEngine> {
    try {
      return LearningEngine.fromSnapshot(JSON.parse(await readFile(this.path, "utf8")) as LearningEngineSnapshot);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") return new LearningEngine();
      throw error;
    }
  }
}
