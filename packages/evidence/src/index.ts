...
import type { Consent, Event, EvidenceRef, Hypothesis, Observation, Scope, Signal, SourceRecord } from "@meraki/contracts";
...
export type ExplicitActivityType = "approval" | "rejection" | "choice" | "correction" | "edit" | "example" | "workflow_action" | "outcome";
...
export const isPromptInjectionSuspected = (content: string): boolean => promptInjectionPatterns.some((pattern) => pattern.test(content));
...
export class EvidenceLedger {
  ...
  private ingestActivity(input: ExplicitActivityInput, allowCanonicalPayload: boolean): EvidenceChain {
    ...
    const source: SourceRecord = deepFreeze({ contract: "source_record", id: randomUUID(), tenant_id: input.tenantId, subject_id: input.subjectId, source_type: `explicit_${input.activityType}`, trust_class: "explicit_user", consent, content_hash: digest(input.content), created_at: recordedAt });
    const event: Event = deepFreeze({ contract: "event", id: randomUUID(), tenant_id: input.tenantId, subject_id: input.subjectId, source_id: source.id, event_type: input.activityType, occurred_at: recordedAt, recorded_at: recordedAt, payload: { actor_id: input.actorId, run_id: input.runId, task_type: input.taskType, content: input.content, scope: input.scope, ...(input.mode === undefined ? {} : { mode: input.mode }), ...(input.payload ?? {}), ...(securityFlags.length ? { security_flags: securityFlags } : {}) }, evidence_spans: [evidenceRef("pending", input.content)] });
  ...
  ingestModelOutput(input: ModelOutputInput): EvidenceChain {
    ...
    const recordedAt = now();
    const source: SourceRecord = deepFreeze({ contract: "source_record", id: randomUUID(), tenant_id: input.tenantId, subject_id: input.subjectId, source_type: "model_output", trust_class: "model_generated", consent: defaultConsent(), content_hash: digest(input.output), created_at: recordedAt });
    const event: Event = deepFreeze({ contract: "event", id: randomUUID(), tenant_id: input.tenantId, subject_id: input.subjectId, source_id: source.id, event_type: "model_output", occurred_at: recordedAt, recorded_at: recordedAt, payload: { run_id: input.runId, output: input.output, scope: input.scope, ...(input.mode === undefined ? {} : { mode: input.mode }) }, evidence_spans: [evidenceRef("pending", input.output)] });
  ...
  observeExplicitActivity(eventId: string): Observation {
    ...
    const source = this.requireSource(event.source_id);
    if (source.trust_class === "model_generated") throw new Error("MODEL_OUTPUT_NOT_USER_EVIDENCE");
    if (source.trust_class !== "explicit_user" && source.trust_class !== "objective_outcome" && source.trust_class !== "observed_behavior") throw new Error("UNTRUSTED_SOURCE");
    if (Array.isArray(event.payload.security_flags) && event.payload.security_flags.includes("prompt_injection_suspected")) throw new Error("POTENTIAL_PROMPT_INJECTION_REVIEW_REQUIRED");
    const existing = [...this.observations.values()].find((observation) => observation.event_ids.length === 1 && observation.event_ids[0] === eventId);
    if (existing) return existing;
    const content = typeof event.payload.content === "string" ? event.payload.content : JSON.stringify(event.payload.outcome ?? event.payload);
    const observation: Observation = deepFreeze({ contract: "observation", id: randomUUID(), tenant_id: event.tenant_id, subject_id: event.subject_id, event_ids: [event.id], description: content, extractor: "deterministic-activity-observer", extractor_version: "1", epistemic_class: source.trust_class === "objective_outcome" ? "deterministic" : "direct", alternatives: [], created_at: now() });
  ...
  signalExplicitActivity(observationId: string, scope: Scope, kind = "explicit_activity"): Signal {
    ...
    const existing = [...this.signals.values()].find((signal) => signal.observation_ids.length === 1 && signal.observation_ids[0] === observationId && signal.kind === kind);
    if (existing) return existing;
    const signal: Signal = deepFreeze({ contract: "signal", id: randomUUID(), tenant_id: observation.tenant_id, subject_id: observation.subject_id, observation_ids: [observation.id], kind, scope, support: 1, counterevidence: 0, confidence: 1, created_at: now() });
  ...
  proposeHypothesis(signalId: string, claim: string): Hypothesis {
    ...
    const event = this.requireEvent(eventId);
    const content = typeof event.payload.correction === "string" ? event.payload.correction
      : typeof event.payload.content === "string" ? event.payload.content
        : JSON.stringify(event.payload.outcome ?? event.payload);
    const hypothesis: Hypothesis = deepFreeze({ contract: "hypothesis", id: randomUUID(), tenant_id: signal.tenant_id, subject_id: signal.subject_id, claim, scope: signal.scope, evidence: [evidenceRef(event.id, content)], alternatives: [], falsifier: "A later explicit correction, rejection, or scoped counterexample contradicts this claim.", confidence: signal.confidence, created_at: now() });
    this.hypotheses.set(hypothesis.id, hypothesis);
    return hypothesis;
  }

  getSource(id: string): SourceRecord { return this.requireSource(id); }
  getEvent(id: string): Event { return this.requireEvent(id); }
  getObservation(id: string): Observation { return this.requireObservation(id); }
  ...
  private requireEvent(id: string): Event { const event = this.events.get(id); if (!event) throw new Error("EVENT_NOT_FOUND"); return event; }
  private requireObservation(id: string): Observation { const observation = this.observations.get(id); if (!observation) throw new Error("OBSERVATION_NOT_FOUND"); return observation; }
  private requireSignal(id: string): Signal { const signal = this.signals.get(id); if (!signal) throw new Error("SIGNAL_NOT_FOUND"); return signal; }
}
